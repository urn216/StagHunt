...
public class EndlessMDP implements MDP {
  ...
  public EndlessMDP(double gamma, Action[] actions, int actorRecieving, int actorDoing) {
    ...
    for (int s = 0; s < sTot; s++) {
      ...
      for (int a = 0; a < actions.length; a++) {
        ...
        if (sPrime == State.Encoder.exitStateEncoded()) {
          // No leaving >:(
          T[s][a][sPrime] =  1;
          R[s][a][sPrime] = COST_OF_FAILING;
          continue;
        }

        // Probability of success constant for now
        T[s][a][sPrime] = 1;
        // Claim reward, keep chugging
